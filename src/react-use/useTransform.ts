...
import { MessageTwoTone, MehTwoTone } from '@ant-design/icons';
...
const moveTrigger: moveType[] = ['dragging', 'resizing', 'builtinResizing'];
    ...
    case 'move':
      ...
      if (moveTrigger.some(mt => prevState.managementData[mt])) {
        ...
        if (prevState.managementData.resizing) {
          ...
          const vec1 = { x: x - centerX, y: y - centerY };
          const vec2 = { x: standardWidth / 2, y: standardHeight / 2 };
          const b = Math.sqrt(Math.pow(vec1.x, 2) + Math.pow(vec1.y, 2));
          const c = standardRadius;
          const a = Math.sqrt(
          ...
          const outerProduct = vec1.x * vec2.y - vec2.x * vec1.y;
          ...
          if (bound) {
            ...
            if (
              (currentTop < parentTop && deltaY < 0) ||
              (currentBottom > parentBottom && deltaY > 0)
            ) {
              deltaY = 0;
            }
          }
          ...
          const vector1 = whichQuadrant(
            { x: prevState.transform.centerX, y: prevState.transform.centerY },
            { x, y },
          );
          const vector2 = { x: deltaX, y: deltaY };
          const action = Zoom(vector1, vector2);
          if (action === 'zoomin') {
            const deltaRadius = Math.round(
              Math.sqrt(
                Math.pow(Math.round(deltaX), 2) +
                  Math.pow(Math.round(deltaY), 2),
              ),
            );
...
const useTransform = ({
  ...
  useEffect(() => {
    ...
    if (current && current.current) {
      ...
      offsetParent =
        parent && parent.current ? parent.current : currentElement.offsetParent;
      ...
      if (offsetParent) {
        ...
        const standardRadius = Math.round(
          Math.sqrt(
            Math.pow(Math.round(standardWidth / 2), 2) +
              Math.pow(Math.round(standardHeight / 2), 2),
          ),
        );
